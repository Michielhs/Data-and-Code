...
# 1 for GPT4o, 0 for GPT3.5
GPT_select = 0
if GPT_select == 1:
    print("warning GPT4o is selected")
...
for index, row in filtered_df.iterrows():
    ...
    risk = row["risk_self"]
    Persona = f"Your willingness to take risks: {risk}, based on a scale from 0 (very risk-averse) to 10 (very risk-seeking)"
    message_history.append({"role": "system", "content": Persona})
    ...
    abbreviation = extract_abbreviation(content_only[2])
    ...
    if explanation == 1:
        filtered_df.at[index, 'explanation_mechanism'] = content_only[2]
    else:
...
for index, row in filtered_df.iterrows():
    ...
    message_history.append({"role": "system", "content": Persona})
    message_history.append({"role": "user", "content": input_text})
    message_history.append({"role": "assistant", "content": row['GroupDecisionVote']})
...
filtered_df.to_csv('RUN10.csv', index=False)
